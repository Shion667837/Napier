class Napier::UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save
      ...
      redirect_to napier_user_path(@user)
    else
      render 'new'
    end
  end
  ...
  def correct_user
    @user = User.find(params[:id])
    redirect_to(napier_root_url) unless current_user?(@user)
  end
end
